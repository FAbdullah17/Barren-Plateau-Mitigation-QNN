"""Local-cost runner: end-to-end training with the local cost function.

Same equal ``total_updates`` budget as the baseline, but ``cost='local'``
(per-qubit readout, Cerezo et al. 2021). One run per seed-triple index.

Usage:
    python experiments/run_local_cost.py configs/local_cost_8layer.yaml --seed-index 0
"""
...
def main():
    parser = argparse.ArgumentParser(
        description='Run a local-cost QNN experiment'
    )
    ...
    config = validate_config(load_config(args.config), approach='local_cost')
